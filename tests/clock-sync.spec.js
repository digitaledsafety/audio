...
test.describe('Clock Synchronization', () => {
  ...
  test('should play first arpeggiator note once when triggered by spacebar or play button', async ({ page }) => {
    // Add Clock and Arpeggiator
    await page.locator('#addNodeToggle').click();
    await page.locator('#addMasterClockNodeBtn').click();
    ...
    // Instrument Arpeggiator.prototype.scheduleNote
    await page.evaluate(() => {
      window.scheduledNoteCalls = [];
      const originalScheduleNote = Arpeggiator.prototype.scheduleNote;
      Arpeggiator.prototype.scheduleNote = function(midiNotes, durationSeconds, startTime) {
        window.scheduledNoteCalls.push({ midiNotes, startTime });
        return originalScheduleNote.apply(this, arguments);
      };
    });

    // Reset array immediately before space bar press
    await page.evaluate(() => { window.scheduledNoteCalls = []; });

    // Press Space bar to start playback
    await page.keyboard.press('Space');
    await page.waitForTimeout(200);

    const callsSpace = await page.evaluate(() => window.scheduledNoteCalls);
    expect(callsSpace.length).toBeGreaterThanOrEqual(1);

    // Stop playback via Space bar
    await page.keyboard.press('Space');
    ...
    const callsPlayBtn = await page.evaluate(() => window.scheduledNoteCalls);
    expect(callsPlayBtn.length).toBeGreaterThanOrEqual(1);
  });
});
